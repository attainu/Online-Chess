...
export const getGameBoardState = (gameId) => async dispatch => {
  axios.get(`https://lichess.org/api/board/game/stream/${gameId}`, {
    headers: {
...
export const handleDrawOffer = (gameId, playerReceiving, accept) => async dispatch => {
    //accept has to be yes or no
    axios.post(`https://lichess.org/api/board/game/${gameId}/draw/${accept}`, {
        headers: {
...
export const abortGame = (gameId, currentPlayer) => async dispatch => {
    axios.post(`https://lichess.org/api/board/game/${gameId}/abort`, {
        headers: {
            Authorization: `Bearer ${currentPlayer === 1 ? token1  : token2}`,
          },
    })
